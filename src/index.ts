...
export type Hero3DInstance = {
  destroy: () => void;
  update: (nextOptions: Partial<Hero3DOptions>) => void;
};

export type Hero3DProjectItem = {
  image: string;
  title?: string;
  href?: string;
  target?: string;
...
export type Hero3DAngles = {
  x: number;
  y: number;
  z: number;
};

export type Hero3DProjectListOptions = {
...
export type Hero3DProjectListInstance = {
  destroy: () => void;
...
type NormalizedHero3DProjectListOptions = Omit<
  ...
  "baseAngles"
> & {
  baseAngles: Hero3DAngles;
};

const DEFAULT_OPTIONS: Required<Hero3DOptions> = {
  maxRotate: 11,
  scaleOnHover: 1.02,
  transitionMs: 320,
  depth: 40,
...
const DEFAULT_PROJECT_LIST_OPTIONS: Omit<
  Required<Hero3DProjectListOptions>,
  "items"
> = {
  baseAngles: {
...
const DEFAULT_PROJECT_LIST_BASE_ANGLES: Hero3DAngles = {
  x: 70,
  y: 0,
  z: 40,
};
...
function applyBaseStyles(
  element: HTMLElement,
  options: Required<Hero3DOptions>,
): void {
...
function ensureGlareLayer(element: HTMLElement): HTMLDivElement {
  let glare = element.querySelector<HTMLDivElement>(":scope > .hero3d-glare");
...
export function createHero3D(
  element: HTMLElement,
  userOptions: Hero3DOptions = {},
): Hero3DInstance {
  if (!element) {
    throw new Error("createHero3D: element is required.");
  }

  let options: Required<Hero3DOptions> = { ...DEFAULT_OPTIONS, ...userOptions };
  let glareLayer: HTMLDivElement | null = null;
  ...
  const update = (nextOptions: Partial<Hero3DOptions>): void => {
...
function normalizeProjectListOptions(
  userOptions: Hero3DProjectListOptions,
): NormalizedHero3DProjectListOptions {
  const baseAngles = {
...
function createCardAnchor(item: Hero3DProjectItem): HTMLAnchorElement {
...
function setProjectListTransform(
  ...
  options: NormalizedHero3DProjectListOptions,
  rotateDeltaX = 0,
  rotateDeltaY = 0,
  rotateDeltaZ = 0,
): void {
...
function applyStackDensityLayout(
  listElement: HTMLElement,
  options: NormalizedHero3DProjectListOptions,
): void {
  const listItems = Array.from(
    listElement.querySelectorAll<HTMLElement>(":scope > .hero3d-project-item"),
  );
...
export function createHero3DProjectList(
  element: HTMLElement,
  userOptions: Hero3DProjectListOptions,
): Hero3DProjectListInstance {
  if (!element) {
  ...
  let listWrapElement: HTMLDivElement | null = null;
  let listElement: HTMLDivElement | null = null;
  const cardDisposers: Array<() => void> = [];

  const clearCardListeners = (): void => {
  ...
  const bindCardHoverShift = (): void => {
    ...
    const cards = listElement.querySelectorAll<HTMLElement>(
      ":scope .hero3d-project-card",
    );
    cards.forEach((card) => {
      const listItem = card.closest<HTMLElement>(".hero3d-project-item");
      const solid = card.querySelector<HTMLElement>(
  ...
  const bindListPointerRotate = (): void => {
    if (options.hoverRotate <= 0 && !options.enableMouseDynamic) {
      element.removeEventListener("pointermove", onPointerMove);
      element.removeEventListener("pointerleave", onPointerLeave);
      return;
  ...
  const update = (nextOptions: Partial<Hero3DProjectListOptions>): void => {
